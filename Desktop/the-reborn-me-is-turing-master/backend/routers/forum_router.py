...
from models import ForumCategory, ForumCheckin, ForumComment, ForumPost, KnowledgeMastery, Mistake, User, UserMemory
...
def ai_answer(post_id: int, db: Session = Depends(get_db)):
    ...
    if post.user_id:
        mastery = (
            ...
            .filter(
                KnowledgeMastery.user_id == post.user_id,
                KnowledgeMastery.subject == post.subject,
                KnowledgeMastery.knowledge_point == post.knowledge_point,
            )
            .first()
        )
        recent = (
            db.query(UserMemory)
            .filter(UserMemory.user_id == post.user_id, UserMemory.status == "active")
        ...
        recent_errors = (
            db.query(Mistake)
            .filter(Mistake.user_id == post.user_id, Mistake.status == "active")
            .order_by(Mistake.create_time.desc())
            .limit(3)
            .all()
        )
        author_profile = {
            "mastery_status": mastery.final_status if mastery else "未知",
            "recent_memories": [
                {"content": m.content, "knowledge_point": m.knowledge_point} for m in recent
            ],
            "common_errors": list({m.error_type for m in recent_errors if m.error_type}),
        }

    return success(ai_answer_for_post(post.title, post.content, post.subject, post.knowledge_point, author_profile))
