...
CREATE TABLE IF NOT EXISTS `question_generation_session` (
  ...
  `generation_mode` ENUM('manual','auto','weak_point') DEFAULT 'manual' COMMENT '出题模式',
  `recommend_mode` VARCHAR(64) DEFAULT '' COMMENT '推荐模式',
  `subject` VARCHAR(64) COMMENT '科目',
  `knowledge_point` VARCHAR(128) COMMENT '知识点',
  `difficulty` ENUM('简单','中等','困难') DEFAULT '中等' COMMENT '难度',
  `question_type` ENUM('选择题','综合题','简答题') DEFAULT '选择题' COMMENT '题型',
...
CREATE TABLE IF NOT EXISTS `question` (
  ...
  `difficulty` ENUM('简单','中等','困难') DEFAULT '中等' COMMENT '难度',
  `question_type` ENUM('选择题','综合题','简答题') DEFAULT '选择题' COMMENT '大题型',
...
CREATE TABLE IF NOT EXISTS `knowledge_mastery` (
  ...
  `subject` VARCHAR(64) COMMENT '科目',
  `knowledge_point` VARCHAR(128) COMMENT '知识点',
  `final_status` ENUM('未学','生疏','一般','熟练') DEFAULT '未学' COMMENT '最终掌握标签',
...
CREATE TABLE IF NOT EXISTS `user_daily_activity` (
  ...
  `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (`user_id`) REFERENCES `user`(`id`) ON DELETE CASCADE,
  UNIQUE KEY `uk_daily_uid_date` (`user_id`),
