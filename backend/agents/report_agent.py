...
def _valid_learning_point(subject: str, knowledge_point: str) -> bool:
    return subject in SUBJECTS and bool((knowledge_point or "").strip()) and knowledge_point != "待生成" and "?" not in knowledge_point
...
def generate_report(db: Session, user_id: int) -> dict:
    ...
    Returns:
        dict: 包含 agent_steps, llm_used, llm_error 的完整报告数据。
    """
    steps: list[dict] = []
    start = time.time()
    ...
    steps.append(_make_step("collect_data", f"answers={total}, mistakes={len(mistakes)}", f"weak={weak_points[:40]}", "success", start))
...
""",
    ...
    steps.append(_make_step("generate_report", f"LLM={llm.used_llm}", f"total={total}, correct={correct}", step_status, t2))
